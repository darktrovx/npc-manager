...
local function Nearby(model, coords, animDict, animName, scenario, options, distance)
	...
	local ped = CreatePed(model, coords.xyz, coords.w, 0, 0)
	...
	while true do
		for i = 1, #Config.Locations, 1 do
			...
			if dist < Config.Distance and not v.rendered then
				local ped = Nearby(v.model, v.coords, v.animDict, v.animName, v.scenario, v.options, v.distance)
				v.ped = ped
				v.rendered = true
			end
			...
			if dist >= Config.Distance and v.rendered then
				...
				if Config.Target and v.options then
...
local function AddPed(model, coords, options, distance, animDict, animName)
	...
	if not distance then distance = 3.5 end
	if not animDict then animDict = nil end
	if not animName then animName = nil end
